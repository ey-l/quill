...
def mask_Qtable(Qtable, cluster_assignments, debug=True):
    ...
    next_attr_start_index = 1
    next_attr_end_index = num_unique_clusters + 1
    for state in range(len(Qtable)):
        if state == next_attr_start_index:
            if debug:
                print("State: ", state)
                print("curr_attr: ", curr_attr)
                print("next_attr_start_index: ", next_attr_start_index)
            curr_attr += 1
            if curr_attr < len(cluster_assignments):
                num_unique_clusters = len(np.unique(cluster_assignments[curr_attr]))
                next_attr_start_index = next_attr_end_index
                next_attr_end_index += num_unique_clusters
            else: next_attr_start_index = next_attr_end_index
        Qtable[state][:next_attr_start_index] = -np.inf
        Qtable[state][next_attr_end_index:] = -np.inf
...
if __name__ == '__main__':
    ...
    learning_rate = 0.3 # to avoid overwriting good alternate paths too aggressively.
    ...
    max_steps = 3
    gamma = 0.99 # high (e.g., 0.95–0.99) to value long-term rewards.

    # Exploration parameters
    max_epsilon = 1.0 
    min_epsilon = 0.5 # high to encourage exploration.
    decay_rate = 0.01 # low to encourage exploration.
    
    # Load ground truth pareto front
    gt_df = pd.read_csv(os.path.join(ppath, 'testresults', f'{dataset}.multi_attrs_exhaustive_search.csv'))
